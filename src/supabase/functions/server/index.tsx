...
app.post('/make-server-71735bdc/chat/:characterId', async (c) => {
  try {
    ...
    // Get all chats
    const allChats = await kv.get(`chat:${user.id}`) || {};
    const chatData = allChats[characterId] || { messages: [], lastRead: null };
    const messages = chatData.messages || [];

    // Add user message with ISO timestamp
    const userMessage = {
      role: 'user',
      content: message,
    ...
    // Get AI response and track response time
    const responseStartTime = Date.now();
    const aiResponse = await getAIResponse(characterId, messages, profile);
    ...
    const assistantMessage: any = {
      ...
      timestamp: formatTimestamp(new Date()),
      responseTime: responseTime // AI 답변 시간 (ms)
    };
    
    // For group chat, include which character responded
...
app.post('/make-server-71735bdc/chat/:characterId/init', async (c) => {
  try {
    ...
    const characters: Record<string, { name: string; greeting: string }> = {
      ...
      'char_3': { name: '레오', greeting: '안녕하세요. 레오예요. 흘러가는 감정 속에서, 진짜 나의 생��을 찾아봐요. 지금 어떤 기분이신가요?' },
    ...
    // Update chat data
    // Set lastRead to greeting timestamp since user is in the chat room
    allChats[characterId] = {
...
const fallbackResponses: Record<string, string[]> = {
  ...
  'char_4': [
    ...
    '일정과 휴식의 ���형이 중요해요. 쉬는 시간도 계획에 넣어봐요.',
...
function selectCharacterForGroupChat(message: string): { charId: string; charName: string; charEmoji: string } {
  ...
  const lumiKeywords = ['힘들', '우울', '외로', '슬프', '불안', '걱정', '두려', '무서', '위로', '공감', '마음', '감정'];
  const kaiKeywords = ['어떻게', '방법', '해결', '계획', '루틴', '습관', '시작', '정리', '관리', '조언', '문제'];
  const leoKeywords = ['왜', '이유', '생각', '의미', '나는', '스스로', '���화', '성찰', '이해', '원인', '진짜'];
